...
# SUSPECT Runtime warning == divide through Zero due to some titles with no-terms.
# THEREFORE extend vocabulary to full argument
class TopicSignatureConstruction(object):
    ...
    def __init__(self, lemma_data_path, output_path):
        ...
        # NOTE STOPWORDS NEED TO BE STRIPPED: "\n"
        self.stopwords = [w.strip("\n").lower() for w in open(filepath/'../data/stopwords.txt')]
...
if __name__=='__main__':
    lemma_path = filepath/'../data/lemma.jsonl'
    output_path = filepath/'../data/topic_signatures.jsonl'
